...
export const MusicPlayer: React.FC<Props> = ({ 
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto p-4 perspective-1000">
      
      {/* Chassis - Improved Depth & Texture */}
      <div className="relative bg-[#d7ccc8] rounded-2xl p-6 shadow-[0_20px_40px_-5px_rgba(62,39,35,0.4),0_8px_10px_-6px_rgba(0,0,0,0.1),inset_0_2px_4px_rgba(255,255,255,0.4)] overflow-hidden border border-[#bcaaa4]">
        ...
        <PencilDandelion 
          className="absolute bottom-8 right-4 w-32 h-32 rotate-[-10deg] opacity-60 mix-blend-multiply z-0" 
          color="#5d4037"
        />
        <PencilDandelion 
        ...
        {/* Stamp Sticker - "PANIC" - Breeze Animation */}
        <div className={`absolute top-4 right-4 z-30 drop-shadow-md origin-top-right transition-transform duration-1000 ${isPlaying ? 'animate-breeze' : 'rotate-6'}`}>
        ...
        {/* Top Section */}
        <div className="flex justify-between items-center mb-6 px-2 relative z-10 mr-16 md:mr-0">
          <div className="w-20 h-12 grid grid-cols-5 gap-1.5 opacity-60">
             {[...Array(20)].map((_, i) => (
               <div key={`l-${i}`} className="bg-[#4e342e] rounded-full w-1.5 h-1.5 shadow-[inset_0_1px_1px_rgba(0,0,0,0.5)]"></div>
             ))}
          </div>
          <div className="text-center relative">
             <div className="text-[#4e342e] font-serif font-bold tracking-widest text-xl border-b-2 border-[#8d6e63]/40 pb-1 drop-shadow-sm">RETRO-SONIC</div>
             <div className="text-[10px] text-[#8d6e63] font-mono mt-1 tracking-[0.2em] uppercase">Analog Series</div>
          </div>
          <div className="w-20 h-12 grid grid-cols-5 gap-1.5 opacity-60">
             {[...Array(20)].map((_, i) => (
        ...
        {/* Main Interface */}
        <div className="bg-[#efebe9] rounded-lg p-5 shadow-[inset_0_2px_8px_rgba(0,0,0,0.05),0_1px_0_rgba(255,255,255,0.8)] border border-white/50 relative z-10">
          
          {/* LCD Screen - More Realistic */}
          <div className="bg-[#78887b] w-full h-28 rounded-sm mb-6 border-4 border-[#8d6e63] shadow-[inset_0_0_15px_rgba(0,0,0,0.4)] relative overflow-hidden flex flex-col items-center justify-center p-4 group">
            {/* Screen Inner Shadow */}
            ...
            {currentTape ? (
              <>
                <div className="flex w-full justify-between items-start mb-auto z-10 w-full relative">
                   <div className="flex flex-col text-[#1a1a1a] w-full">
                     <h2 className="text-lg font-mono font-bold truncate w-full tracking-tighter drop-shadow-sm opacity-90">{currentTape.title || "Unknown Track"}</h2>
                     <h3 className="text-xs font-mono text-[#2e4c2e] truncate w-full opacity-80">{currentTape.artist || "Unknown Artist"}</h3>
                   </div>
                </div>

                <div className="flex w-full justify-between items-end z-10 relative">
                    {/* Visualizer */}
                    <div className="flex items-end h-8 gap-0.5 opacity-80 flex-1 mr-4">
                    ...
                    <div className="text-right font-mono text-[#1a1a1a] text-xl tracking-widest opacity-90">
                        {formatTime(currentTime)}
                    </div>
                </div>
              </>
            ) : (
              <div className="text-[#2e4c2e]/60 font-mono text-sm tracking-widest z-10 animate-pulse">
                NO TAPE
              </div>
            )}
          ...
          {/* Tape Deck Window */}
          <div className="bg-[#3e2723] w-full h-36 rounded-sm mb-6 relative overflow-hidden border-2 border-[#5d4037] shadow-[inset_0_6px_20px_rgba(0,0,0,0.6)] flex items-center justify-center">
            {/* Glass Reflection */}
            <div className="absolute inset-0 bg-gradient-to-tr from-white/5 via-white/0 to-white/10 pointer-events-none z-20"></div>
            
            {currentTape ? (
              <div className={`relative w-44 h-28 bg-[#2b2b2b] rounded flex items-center justify-center transform transition-transform duration-700 ${isPlaying ? 'scale-[1.01]' : 'scale-100'} shadow-[0_4px_10px_rgba(0,0,0,0.5)]`}>
                 {/* Tape Texture */}
                 <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/dark-leather.png')] opacity-50"></div>
                 
                 {/* Reels */}
                 <div className={`absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border-[3px] border-[#757575] flex items-center justify-center ${isPlaying ? 'animate-spin-slow' : ''} bg-[#1a1a1a]`}>
                 ...
                 <div className={`absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border-[3px] border-[#757575] flex items-center justify-center ${isPlaying ? 'animate-spin-slow' : ''} bg-[#1a1a1a]`}>
                    <div className="w-1.5 h-1.5 bg-[#757575] rounded-full"></div>
                    <div className="absolute w-full h-0.5 bg-[#757575]"></div>
                    <div className="absolute h-full w-0.5 bg-[#757575]"></div>
                 </div>
                 ...
                 {/* Tape Window */}
                 <div className="h-8 w-20 bg-[#1a1a1a] rounded-sm relative overflow-hidden border border-[#444] shadow-inner">
                    <div className="w-full h-full bg-gradient-to-r from-[#111] via-[#2a2a2a] to-[#111] opacity-90"></div>
                 </div>

                 {/* Tape Label Strip */}
                 <div className={`absolute bottom-3 left-1/2 -translate-x-1/2 w-28 h-5 rounded-sm opacity-90 shadow-sm
          ...
          <div className="flex justify-center gap-8 items-center">
            <button 
              ...
              className="group w-14 h-14 rounded-full bg-gradient-to-b from-[#efebe9] to-[#d7ccc8] shadow-[0_4px_0_#a1887f,0_6px_10px_rgba(0,0,0,0.15)] active:shadow-[0_2px_0_#a1887f,inset_0_2px_5px_rgba(0,0,0,0.1)] active:translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center border border-[#fff]"
            >
               <Square className="text-[#5d4037] fill-[#5d4037]" size={18} />
            </button>

            <button 
              onClick={onPlayPause}
              disabled={!currentTape}
              className="group w-20 h-20 rounded-full bg-gradient-to-b from-[#ffe0b2] to-[#ffcc80] shadow-[0_5px_0_#ef6c00,0_8px_15px_rgba(239,108,0,0.25)] active:shadow-[0_2px_0_#ef6c00,inset_0_2px_5px_rgba(0,0,0,0.1)] active:translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center border border-[#fff3e0]"
            >
               {isPlaying ? <Pause className="text-[#bf360c] fill-[#bf360c]" size={32} /> : <Play className="text-[#bf360c] fill-[#bf360c] ml-1" size={32} />}
            </button>
            
            <button 
              className="group w-14 h-14 rounded-full bg-gradient-to-b from-[#efebe9] to-[#d7ccc8] shadow-[0_4px_0_#a1887f,0_6px_10px_rgba(0,0,0,0.15)] active:shadow-[0_2px_0_#a1887f] active:translate-y-1 opacity-60 cursor-not-allowed transition-all flex items-center justify-center border border-[#fff]"
            >
               <Music4 className="text-[#5d4037]" size={20} />
            </button>
          </div>
